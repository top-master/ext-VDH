...
(() => {
  ...
  weh.is_safe.then(() => {
    let { info: versionExport } = (initVersion(), toCommonjs(versionNs));
    initLicPanel();
        ...
        return React.createElement(
          ...
          React.createElement(
            'div',
            {
              className:
                'section-header '
                + (this.state.open ? 'section-open' : 'section-close'),
              onClick: this.toggle(),
            },
            React.createElement(
              'div',
          ...
          React.createElement(
            ...
            React.createElement(
              Card,
              null,
              React.createElement(CardBody, null, this.props.content),
            ),
          ),
        );
      }
    ...
    window.AddonInfoPanel = class extends React.Component {
      constructor(props) {
        ...
        var self = this;
        weh.rpc.call('getBuild').then(build => {
          self.setState({
            build: build,
          });
        });
      ...
      render() {
        var self = this;
        let version =
          this.state.manifest.version_name || this.state.manifest.version;
        let channelSuffix = this.state.build
          ? ' (' + this.state.build.channel + ')'
          : '';
        let content = React.createElement(
          ...
          React.createElement(
            'div',
            null,
            weh._('version', version) + channelSuffix,
          ),
          React.createElement(
            'div',
            null,
            weh._('browser_locale', browser.i18n.getUILanguage()),
          ),
          this.state.build
            && React.createElement(
              'div',
              ...
              React.createElement(
                'div',
                null,
                weh._('built_on', this.state.build.buildDate),
              ),
                    ...
                    Object.keys(this.state.build.buildOptions)
                      ...
                      .map(
                        optionKey =>
                          optionKey
                          + '='
                          + self.state.build.buildOptions[optionKey],
                      )
                      .join(', '),
                  ),
                ),
        ...
        let title = this.state.manifest.name + ' ' + version + channelSuffix;
        ...
        let content = React.createElement(
          ...
          this.state.platform
            && React.createElement(
              ...
              null,
              weh._('platform_info', [
                this.capitalize(this.state.platform.os),
                this.state.platform.arch,
              ]),
            ),
          (this.state.browser
            && React.createElement(
              ...
              null,
              weh._('browser_info', [
                this.state.browser.vendor,
                this.state.browser.name,
                this.state.browser.version,
              ]),
            ))
    ...
    window.CoAppInfoPanel = class extends React.Component {
      ...
      async check(restart) {
        ...
        Promise.resolve()
          ...
          .then(() => {
            weh.rpc.call('checkCoApp').then(status => {
              self.setState(
                Object.assign(
                  {
                    status: null,
      ...
      isMinimumVersion(version, minVersion) {
        for (
          var versionParts = version.split('.').map(part => parseInt(part)),
            minParts = minVersion.split('.').map(part => parseInt(part)),
            index = 0;
          index < versionParts.length;
          index++
        ) {
          if (
            typeof minParts[index] > 'u'
            || versionParts[index] > minParts[index]
          ) {
            return !0;
          }
          if (versionParts[index] < minParts[index]) {
            return !1;
          }
        }
        return !0;
      ...
      render() {
        ...
        if (this.state.status === null) {
          statusLabel = weh._('coapp_unchecked');
        } else {
          if (this.state.status) {
            if (
              this.isMinimumVersion(
                this.state.info.version,
                versionExport.lastVersion,
              )
            ) {
              statusLabel = weh._('coapp_installed');
            } else {
              statusLabel = weh._('coapp_outdated');
              isOutdated = !0;
            }
          } else {
            statusLabel = weh._('coapp_not_installed');
          }
        }
        var panel = React.createElement(
          'div',
          ...
          this.state.error
            && React.createElement(
              ...
              React.createElement(
                ...
                {
                  target: '_blank',
                  href: globalThis.extConfig.getUrlValue('coappHelpUrl'),
                },
                weh._('coapp_help'),
              ),
            ),
            ...
            && React.createElement(
              ...
              isOutdated
                && React.createElement(
                  'div',
                  null,
                  weh._('coapp_latest_version', versionExport.lastVersion),
                ),
              React.createElement(
                'div',
                null,
                weh._('coapp_path'),
                '  ',
                React.createElement('em', null, this.state.info.binary),
              ),
            ),
          React.createElement('br', null),
          React.createElement(
