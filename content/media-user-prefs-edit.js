...
(function (someNamespace) {
  function allSome() {
    ...
    for (
      var someValues = [], cursor = 0, optionList = optionArgs;
      cursor < optionList.length;
      cursor++
    ) {
      var option = optionList[cursor];
      if (option.isSome()) {
        someValues.push(option.value);
      } else {
        return option;
      }
    }
  ...
  function anySome() {
    ...
    for (
      var cursor = 0, optionList = optionArgs;
      cursor < optionList.length;
      cursor++
    ) {
      var option = optionList[cursor];
...
var ErrResultImpl = (function () {
  function ErrClass(errorValue) {
    if (!(this instanceof ErrClass)) {
      return new ErrClass(errorValue);
    }
...
var OkResultImpl = (function () {
  function OkClass(okValue) {
    if (!(this instanceof OkClass)) {
      return new OkClass(okValue);
    }
    this.value = okValue;
  }
...
(function (resultNamespace) {
  function allOk() {
    ...
    for (
      var okValues = [], cursor = 0, resultList = resultArgs;
      cursor < resultList.length;
      cursor++
    ) {
  ...
  function anyOk() {
    ...
    for (
      var errorValues = [], cursor = 0, resultList = resultArgs;
      cursor < resultList.length;
      cursor++
    ) {
      var result = resultList[cursor];
...
var runGenerator = function (thisArg, bodyFn) {
  ...
  function step(operation) {
    ...
    for (; iterator && ((iterator = 0), operation[0] && (state = 0)), state; ) {
      try {
        if (
          ((isExecuting = 1),
          delegate
            && (stepTuple =
              operation[0] & 2
                ...
                : operation[0]
                  ? delegate.throw
                    || ((stepTuple = delegate.return)
                      && stepTuple.call(delegate),
                    0)
                  : delegate.next)
            && !(stepTuple = stepTuple.call(delegate, operation[1])).done)
        ) {
          return stepTuple;
        }
        switch (
          ((delegate = 0),
          stepTuple && (operation = [operation[0] & 2, stepTuple.value]),
          operation[0])
        ) {
          case 0:
          case 1:
          ...
          default:
            if (
              ((stepTuple = state.trys),
              !(stepTuple =
                stepTuple.length > 0 && stepTuple[stepTuple.length - 1])
...
var AsyncResult = (function () {
  ...
  AsyncResultClass.prototype.andThen = function (mapper) {
    var self = this;
      ...
      return runAwaiter(self, void 0, void 0, function () {
        ...
        return runGenerator(this, function (genContext) {
          if (result.isErr()) {
            ...
            nextResult = mapper(result.value);
            return [
              2,
              nextResult instanceof AsyncResultClass
                ? nextResult.promise
                : nextResult,
            ];
          }
