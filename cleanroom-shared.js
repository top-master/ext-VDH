...
(() => {
  ...
  // The extension's own pages live at chrome-extension://<id>/..., and this
  // build carries the upstream extension id - so the bare chromeListingId /
  // edgeListingId fingerprints match its own origin. Never treat the
  // extension's own resources (e.g. content/about.html, content/settings.html)
  // as an upstream URL: blocking them would break every button that opens a
  // local page.
  function ownExtensionUrlPrefix() {
    const runtimeRoot =
      globalThis.browser?.runtime || globalThis.chrome?.runtime || null;
    if (runtimeRoot && typeof runtimeRoot.getURL == "function") {
      try {
        return runtimeRoot.getURL("/");
      } catch {}
    }
    return "";
  }

  function isOwnExtensionUrl(candidate) {
    const prefix = ownExtensionUrlPrefix();
    if (!prefix) {
      return false;
    }
    const normalizedCandidate = normalizeUrlCandidate(candidate).toLowerCase();
    return normalizedCandidate.startsWith(prefix.toLowerCase());
  }

  function shouldBlockOriginalUrl(candidate) {
    return (
      getIsCleanRoom() &&
      !isOwnExtensionUrl(candidate) &&
      isOriginalVdhUrl(candidate)
    );
  }
  ...
  function installDocumentClickGuard() {
    ...
    const blockOriginalAnchorClick = (event) => {
      ...
      // data-cleanroom-url links resolve on click via event delegation, so links
      // added later (e.g. cloned out of a <template>) work too - installing the
      // href up front only covers links already live at load.
      const cleanRoomUrlName = anchorElement.getAttribute("data-cleanroom-url");
      if (cleanRoomUrlName) {
        event.preventDefault();
        event.stopImmediatePropagation();
        const resolvedUrl = getUrlValue(cleanRoomUrlName);
  ...
  function installI18nGetMessageGuard(apiRoot) {
    ...
    const wrappedGetMessage = function(...args) {
      ...
      try {
        // resolve the appName message too: it holds the :appName token, which
        // must expand to the configured brand (getLiteralValue("productName"))
        // rather than be used verbatim as the substitution value.
        localizedAppName = resolveLocaleText(originalGetMessage.call(this, "appName"));
      } catch {}
