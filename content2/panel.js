...
(function (optionNs) {
  function collectAll() {
    for (var args = [], argIndex = 0; argIndex < arguments.length; argIndex++) {
      args[argIndex] = arguments[argIndex];
    }
    for (
      var collected = [], itemIndex = 0, items = args;
...
var ErrImpl = (function () {
  function ErrClass(error) {
    ...
    this.error = error;
    var stackFrames = new Error().stack
      .split(
        `
`,
      )
    ...
    if (
      ...
      && stackFrames[0].includes('ErrImpl')
    ) {
      stackFrames.shift();
    }
    this._stack = stackFrames.join(`
`);
  }
...
(function (resultNs) {
  ...
  function firstOk() {
    ...
    for (
      var errors = [], itemIndex = 0, items = args;
      itemIndex < items.length;
      itemIndex++
    ) {
      var result = items[itemIndex];
...
var runGenerator = function (thisArg, body) {
  ...
  function stepGenerator(genOp) {
    ...
    for (
      ;
      iteratorApi && ((iteratorApi = 0), genOp[0] && (state = 0)), state;
    ) {
      try {
        if (
          ((executing = 1),
          current
            && (opResult =
              genOp[0] & 2
                ...
                : genOp[0]
                  ? current.throw
                    || ((opResult = current.return) && opResult.call(current),
                    0)
                  : current.next)
            && !(opResult = opResult.call(current, genOp[1])).done)
        ) {
          return opResult;
        }
        switch (
          ((current = 0),
          opResult && (genOp = [genOp[0] & 2, opResult.value]),
          genOp[0])
        ) {
          case 0:
          case 1:
          ...
          default:
            ...
            if (
              genOp[0] === 3
              && (!opResult
                || (genOp[1] > opResult[0] && genOp[1] < opResult[3]))
            ) {
              state.label = genOp[1];
              break;
            }
...
var AsyncResultImpl = (function () {
  ...
  AsyncResultClass.prototype.andThen = function (thenFn) {
    var self = this;
