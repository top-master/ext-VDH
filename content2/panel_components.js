...
function localizeMessage(messageKey, substitutions, localMessages) {
  ...
  try {
    if (messageKey in localMessages) {
      ...
      let browserMessage = polyfillNamespace.default.i18n.getMessage(
        messageKey,
        substitutions,
      );
...
function formatBytes(byteCount) {
  if (byteCount < 1048576) {
    ...
    return `${(byteCount / 1048576).toFixed(0)}Mb`;
  }
}
function formatDuration(totalSeconds) {
  ...
  let hoursText = ('0' + hours + ':').slice(-3);
  let minutesText = ('0' + minutes + ':').slice(-3);
  let secondsText = ('0' + seconds).slice(-2);
  if (hoursText == '00:') {
    hoursText = '';
  }
...
function UpdateProgress(downloadableId, progressInfo) {
  ...
  let progressBar = downloadElement.querySelector('sl-progress-bar');
  let percentSpan = downloadElement.querySelector('.span-percent');
  let bitrateSpan = downloadElement.querySelector('.span-bitrate');
  let durationSpan = downloadElement.querySelector(
    '.span-downloading-duration',
  );
  downloadElement.classList.remove('progress-unknown');
  if (typeof progressInfo != 'string') {
    durationSpan.textContent =
      formatDuration(progressInfo.duration_since_start / 1e3) + 's';
    downloadElement.querySelector('.button-stop').removeAttribute('loading');
    downloadElement.querySelector('.button-stop').removeAttribute('disabled');
    bitrateSpan.removeAttribute('hidden');
    bitrateSpan
      .querySelector('sl-format-bytes')
      .setAttribute('value', progressInfo.bitrate_bs.toString());
    if (progressInfo.progress != 'unknown') {
      percentSpan.removeAttribute('hidden');
      percentSpan
        .querySelector('sl-format-number')
        .setAttribute('value', progressInfo.progress.toString());
      progressBar.removeAttribute('indeterminate');
      progressBar.setAttribute(
        'value',
        (progressInfo.progress * 100).toString(),
      );
    } else {
      percentSpan.setAttribute('hidden', 'true');
      progressBar.removeAttribute('value');
      progressBar.setAttribute('indeterminate', 'true');
      downloadElement.classList.add('progress-unknown');
    }
  } else {
    downloadElement
...
function renderVariant(variant, localMessages) {
  ...
  variantParts.push(`<span class="variant-container">${containerExt}</span>`);
  let differentiatorText = localizeMessage(
  ...
  if (coreMedia.av.video) {
    ...
    if (coreMedia.av.video.dimensions.isSome()) {
      ...
      differentiatorText = `${dimensions.width}x${dimensions.height}`;
      if (coreMedia.av.video.bitrate.isSome()) {
        differentiatorText += ` ${formatBytes(coreMedia.av.video.bitrate.unwrap())}/s`;
      }
    } else {
      if (coreMedia.content_length.isSome()) {
        differentiatorText = formatBytes(coreMedia.content_length.unwrap());
      } else {
        if (coreMedia.av.video.bitrate.isSome()) {
          differentiatorText += `${formatBytes(coreMedia.av.video.bitrate.unwrap())}/s`;
        }
  ...
  variantParts.push(
    `<span class="variant-differentiator">${differentiatorText}</span>`,
  );
...
function UpdateDefaultAction(downloadElement, actionName) {
  ...
  let actionMenuItem = downloadElement.querySelector(
    'sl-menu-item.action-' + actionName,
  );
  let iconName = actionMenuItem.querySelector('sl-icon').getAttribute('name');
  let labelText = actionMenuItem.querySelector('span').textContent;
  let actionButton = downloadElement.querySelector(
    '.button-group-actions > sl-button',
  );
  for (let otherAction of downloadActionNames) {
    actionButton.classList.remove('action-' + otherAction);
  }
  actionButton.classList.add('action-' + actionName);
  actionButton.querySelector('sl-icon').setAttribute('name', iconName);
  actionButton.querySelector('span').textContent = labelText;
}
function RenderVariantButton(downloadable) {
  let containerExt = downloadable.core_media.container.extension;
  if (!downloadable.core_media.av.video) {
    containerExt = downloadable.core_media.container.audio_only_extension;
  }
  let containerHtml = `<span class="variant-container">${containerExt}</span>`;
  let videoInfo = downloadable.core_media.av.video;
  let buttonHtml = containerHtml;
  if (videoInfo && videoInfo.quality.isSome()) {
    let qualityLabel = videoInfo.quality.unwrap() + 'p';
    let qualityHtml = `<span class="variant-quality _${qualityLabel}">${qualityLabel}</span>`;
...
function renderDownloadable(downloadable, localMessages) {
  ...
  let downloadRow = document.createElement('hbox');
  downloadRow.classList.add('download');
  downloadRow.dataset.selectedVariantId = firstVariant.id;
  downloadRow.dataset.downloadableId = downloadable.id;
  downloadRow.dataset.timestamp = downloadable.timestamp.toString();
  downloadRow.dataset.toCopy = firstVariant.to_copy;
  let leftHtml;
  {
  ...
  let topHtml;
  {
    ...
    let mediaIconsHtml = '<sl-icon name="film"></sl-icon>';
  ...
  let bottomHtml;
  {
    ...
    for (let variant of downloadable.variants.values()) {
      variantsHtml += renderVariant(variant, localMessages);
    }
    bottomHtml = `
  ...
  {
    let actionsGroup = templateContent
      .querySelector('.button-group-actions')
      .cloneNode(!0);
    let actionsMenu = templateContent
      .querySelector('.menu-actions')
      .cloneNode(!0);
    let downloadBottom = downloadRow.querySelector('.download-bottom');
    downloadBottom.appendChild(actionsGroup);
    downloadBottom.appendChild(actionsMenu);
  }
  applyI18n(downloadRow, localMessages);
  return downloadRow;
}
function UpdateDownloading(downloadingInfo, localMessages) {
  ...
  if (!downloadElement) {
    downloadElement = renderDownloadable(
      downloadingInfo.downloadable,
      localMessages,
    );
    document.querySelector('#core-downloads').append(downloadElement);
  }
  downloadElement.setAttribute('status', 'downloading');
  UpdateProgress(downloadingInfo.downloadable.id, downloadingInfo.progress);
...
function UpdateDownloadable(downloadable, localMessages) {
  ...
  if (!downloadElement) {
    downloadElement = renderDownloadable(downloadable, localMessages);
    document.querySelector('#core-downloads').append(downloadElement);
  }
  downloadElement.setAttribute('status', 'downloadable');
  {
    let selectedVariantId = downloadElement.dataset.selectedVariantId;
    let selectedVariant = downloadable.variants.get(selectedVariantId);
    let variantButtonHtml = RenderVariantButton(selectedVariant);
    let dropdownButton = downloadElement.querySelector(
...
function UpdateDownloaded(downloadedInfo, localMessages) {
  ...
  if (!downloadElement) {
    downloadElement = renderDownloadable(
      downloadedInfo.downloadable,
      localMessages,
    );
    document.querySelector('#core-downloads').append(downloadElement);
  }
  downloadElement.setAttribute('status', 'downloaded');
  let titleElement = downloadElement.querySelector('.title');
  if (downloadedInfo.download_result.inbrowser) {
    downloadElement.dataset.inBrowserDownloadId =
      downloadedInfo.download_result.download_id.toString();
    titleElement.textContent = downloadedInfo.downloadable.title;
  } else {
    titleElement.textContent = downloadedInfo.download_result.filename;
  }
  return downloadElement;
}
export {
