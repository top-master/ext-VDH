...
(function (optionNamespace) {
  function collectAll() {
    for (var options = [], index = 0; index < arguments.length; index++) {
      options[index] = arguments[index];
    }
    for (
      var values = [], cursor = 0, optionsList = options;
      cursor < optionsList.length;
      cursor++
    ) {
  ...
  function pickAny() {
    ...
    for (
      var cursor = 0, optionsList = options;
      cursor < optionsList.length;
      cursor++
    ) {
      var option = optionsList[cursor];
...
(function (resultNamespace) {
  function collectResults() {
    for (var results = [], index = 0; index < arguments.length; index++) {
      results[index] = arguments[index];
    }
    for (
      var values = [], cursor = 0, resultsList = results;
      cursor < resultsList.length;
      cursor++
    ) {
  ...
  function firstOk() {
    ...
    for (
      var errors = [], cursor = 0, resultsList = results;
      cursor < resultsList.length;
      cursor++
    ) {
      var result = resultsList[cursor];
...
var runGenerator = function (thisArg, body) {
  var state = {
    label: 0,
    sent: function () {
      if (tmp[0] & 1) {
  ...
  function step(opPair) {
    ...
    for (
      ;
      controller && ((controller = 0), opPair[0] && (state = 0)), state;
    ) {
      try {
        if (
          ((executing = 1),
          iterator
            && (tmp =
              opPair[0] & 2
                ...
                : opPair[0]
                  ? iterator.throw
                    || ((tmp = iterator.return) && tmp.call(iterator), 0)
                  : iterator.next)
            && !(tmp = tmp.call(iterator, opPair[1])).done)
        ) {
          return tmp;
        }
        switch (
          ((iterator = 0),
          tmp && (opPair = [opPair[0] & 2, tmp.value]),
          opPair[0])
        ) {
          case 0:
          case 1:
          ...
          case 5:
            ...
            continue;
          case 7:
            opPair = state.ops.pop();
            state.trys.pop();
            continue;
          default:
            if (
              ((tmp = state.trys),
              !(tmp = tmp.length > 0 && tmp[tmp.length - 1])
                && (opPair[0] === 6 || opPair[0] === 2))
            ) {
              state = 0;
              continue;
            }
            if (
              opPair[0] === 3
              && (!tmp || (opPair[1] > tmp[0] && opPair[1] < tmp[3]))
            ) {
...
var ResultAsync = (function () {
  ...
  ResultAsyncClass.prototype.andThen = function (mapFn) {
    var self = this;
