...
describe('vendored React', () => {
  ...
  it.each([
    ...
    'Children',
    'createFactory',
  ])('provides React.%s', member =>
    expect(windowObject.React[member]).toBeDefined(),
  );
  it('createElement builds a valid element', () => {
    const element = windowObject.React.createElement(
      'div',
      { className: 'x' },
      'hi',
    );
...
describe('vendored ReactDOM', () => {
  it.each([
    ...
    'version',
  ])('provides ReactDOM.%s', member =>
    expect(windowObject.ReactDOM[member]).toBeDefined(),
  );
  it('window.render is ReactDOM.render', () =>
    expect(windowObject.render).toBe(windowObject.ReactDOM.render));
});
...
describe('vendored Redux', () => {
  it.each([
    ...
    'bindActionCreators',
    'compose',
  ])('provides Redux.%s', member =>
    expect(typeof windowObject.Redux[member]).toBe('function'),
  );
  ...
  it('combineReducers composes slices', () => {
    const rootReducer = windowObject.createStore
      ? windowObject.Redux.combineReducers({
          a: (state = 'A') => state,
          b: (state = 'B') => state,
        })
      : null;
...
describe('full React + Redux + react-redux + ReactDOM render (nothing missing)', () => {
  it('renders a connected component and reflects dispatched state', () => {
    ...
    const CountView = (props: { count: number }) =>
      React.createElement('span', { id: 'count' }, String(props.count));
    const ConnectedCount = windowObject.connect((state: any) => ({
      count: state.count,
    }))(CountView);
    const application = React.createElement(
      windowObject.Provider,
      { store },
      React.createElement(ConnectedCount),
    );
...
describe('vendored prop-types', () => {
  it.each([
    ...
    'oneOf',
    'shape',
  ])('provides PropTypes.%s', member =>
    expect(windowObject.PropTypes[member]).toBeDefined(),
  );
});

describe('vendored classnames', () => {
  it('window.classNames joins truthy names', () => {
    expect(windowObject.classNames('a', { b: true, c: false }, ['d'])).toBe(
      'a b d',
    );
  });
});
...
describe('vendored react-resize-detector (extracted to vendor/react-resize-detector.js)', () => {
  it('window.ReactResizeDetector is a component/HOC', () => {
    expect(['function', 'object']).toContain(
      typeof windowObject.ReactResizeDetector,
    );
...
describe('vendored webextension-polyfill', () => {
  it.each(['runtime', 'storage', 'i18n', 'tabs'])(
    'browser.%s is present',
    member => expect(windowObject.browser[member]).toBeDefined(),
  );
