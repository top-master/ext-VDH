...
export function pageVendorScripts(pageFile: string): string[] {
  ...
  return [...html.matchAll(/<script src="((?:\.\.\/)?vendor\/[^"]+\.js)">/g)].map((match) => match[1]);
}
...
export function extensionStub() {
  ...
  const port = { name: "", onMessage: { ...listener }, onDisconnect: { ...listener }, postMessage: noop, disconnect: noop };
  const storageArea = {
    get: (...callArgs: unknown[]) => {
      const callback = callArgs[callArgs.length - 1];
      if (typeof callback === "function") (callback as (value: unknown) => void)({});
  ...
  return {
    runtime: {
      ...
      getManifest: () => ({ version: "9.0.0" }), getURL: (urlPath: string) => urlPath,
      connect: () => port, onMessage: { ...listener }, onConnect: { ...listener },
      sendMessage: (...callArgs: unknown[]) => { const callback = callArgs[callArgs.length - 1]; if (typeof callback === "function") (callback as () => void)(); },
    },
    storage: { local: storageArea, sync: storageArea, managed: storageArea, onChanged: { ...listener } },
    tabs: { query: () => Promise.resolve([]), onUpdated: { ...listener }, onRemoved: { ...listener } },
    i18n: { getMessage: () => "" },
  };
}
...
export function pageLocalScripts(pageFile: string): string[] {
  const html = readFileSync(resolve(pageFile), "utf8");
  return [...html.matchAll(/<script src="([^"]+)">/g)].map((match) => match[1]).filter((src) => !/^https?:/.test(src));
}
...
export function loadFullPage(pageFile: string, panelName = "test-panel") {
  ...
  windowObject.document.dispatchEvent(new windowObject.Event("DOMContentLoaded", { bubbles: true }));
