...
(() => {
  ...
  var requireI18n = defineCommonjsModule((i18nExports, i18nModule) => {
    ...
    var { browser: i18nBrowser } = requireWehCore();
    var customStrings = {};
    var placeholderRegex = new RegExp('\\$[a-zA-Z]*([0-9]+)\\$', 'g');
    var customStringsLoaded = !1;
    var customStringsReady = i18nBrowser.storage.local
      .get('wehI18nCustom')
      .then(stored => {
    ...
    function getMessage(messageName, substitutions) {
      ...
      try {
        if (substitutions) {
          return i18nBrowser.i18n.getMessage(messageName, substitutions);
        } else {
          return i18nBrowser.i18n.getMessage(messageName);
        }
      } catch {
        return '';
      }
  ...
  var videoIdRegex = new RegExp('\\bv=([^&]+)');
  function collectSelectedVideos() {
    ...
    for (let rangeIndex = 0; rangeIndex < selection.rangeCount; rangeIndex++) {
      ...
      Array.from(links).forEach(link => {
        if (!range.isPointInRange(link, 0)) {
          return;
        }
        let href = link.getAttribute('href');
        if (href && href.length > 0) {
          let match = videoIdRegex.exec(href);
          if (match) {
            videoIds[match[1]] = 1;
          }
        }
    ...
    let topUrl = window._$vdhTopUrl;
    try {
      topUrl = window.top.location.href;
    } catch {}
    let ids = Object.keys(videoIds);
    if (ids.length > 0) {
      browser.runtime.sendMessage({
        yt_selection: ids,
      });
    }
    weh.rpc.call('tbvwsSelectedIds', {
      pageUrl: window.location.href,
      topUrl: topUrl,
      ids: Object.keys(videoIds),
    });
  }
  var bulkPanel = null;
  document.addEventListener('selectionchange', function () {
    if (bulkPanel) {
      clearTimeout(bulkPanel);
    }
    bulkPanel = setTimeout(() => {
      collectSelectedVideos();
    }, 25);
  });
  collectSelectedVideos();
  function showBulkPanel() {
    ...
    buttonElement.appendChild(
      document.createTextNode(weh._('bulk_in_progress')),
    );
    panelElement.appendChild(buttonElement);
    setInterval(() => {
      if (!panelElement.parentNode && document.body) {
  ...
  if (/vdh\-bulk=/.test(window.location.href)) {
    showBulkPanel();
  }
})();
