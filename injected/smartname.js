...
(() => {
  ...
  var xpathHelper = {
    getElementTreeXPath: node => {
      ...
      for (
        ...
        let position = index || hasFollowing ? '[' + (index + 1) + ']' : '';
    ...
    getElementXPath: node =>
      node && node.id
        ? '//*[@id="' + node.id + '"]'
        : xpathHelper.getElementTreeXPath(node),
  };
  var selecting = !1;
  function onSelectionChange() {
    return weh.rpc
        ...
        () =>
          new Promise((resolve, reject) => {
            ...
            if (selection.rangeCount > 0) {
              ...
              resolve(xpathHelper.getElementXPath(container));
            } else {
              resolve(null);
            }
          }),
      ...
      .then(
        xpath =>
          (xpath && ['page-content', xpath]) || [
            'page-title',
            '/html/head/title/text()',
          ],
      )
      .then(([mode, value]) =>
        weh.rpc.call('setSmartNameData', {
          mode: mode,
          xpath: value,
  ...
  function startSelectionListener() {
    if (selectionListener) {
      clearTimeout(selectionListener);
    }
    if (!selecting) {
      selectionListener = setTimeout(() => {
        onSelectionChange();
      }, 250);
    }
  }
  ...
  weh.rpc.listen({
    ...
    select: xpath => {
      ...
      if (xpathResult) {
        ...
        range.setStart(xpathResult, 0);
        range.setEndAfter(xpathResult);
