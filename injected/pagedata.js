...
(() => {
  ...
  function collectPageData() {
    ...
    for (let index = 0; index < selectors.length && !thumbnail; index++) {
      let selector = selectors[index];
      let element = document.querySelector(selector.sel);
      if (element) {
        thumbnail = element.getAttribute(selector.attr) || null;
      }
    }
    if (thumbnail) {
      ...
      let canonical =
        /^(https?:\/\/(?:[^\/]*\.)?\x79\x6f\x75\x74\x75\x62\x65(?:\.co)?\.(?:[^\.\/]+))\/.*\bv=([^&]+)/.exec(
          window.location.href,
        );
      if (canonical) {
        thumbnail = 'https://i.ytimg.com/vi/' + canonical[2] + '/default.jpg';
      }
      thumbnailUrl =
        document.querySelector('head title')?.firstChild?.nodeValue;
    }
    if (thumbnail) {
      ...
      if (videos.length == 1 && videos[0].poster) {
        data.thumbnailUrl = videos[0].poster;
      }
    }
    if (thumbnailUrl) {
  ...
  if (pageData.thumbnailUrl) {
    pageInfo.thumbnailUrl = pageData.thumbnailUrl;
  }
  if (pageData.title) {
    pageInfo.title = pageData.title;
  }
  var hitId = window._$vdhHitId;
  var smartNameSpecs = window._$vdhSmartNameSpecs;
  Promise.resolve()
    .then(() => {
      if (smartNameSpecs) {
        return new Promise((resolve, reject) => {
          setTimeout(() => {
            resolve(evaluateSmartNameHandler(smartNameSpecs));
          }, smartNameSpecs.delay);
        });
      }
    })
    .then(smartName => {
      if (smartName) {
        pageInfo.title = smartName;
      }
      weh.rpc.call('hitPageData', {
        id: hitId,
        data: pageInfo,
      });
    });
