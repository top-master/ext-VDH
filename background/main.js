...
(() => {
    ...
    (polyfillExports, polyfillModule) => {
      (function (globalScope, defineFactory) {
        if (typeof define == 'function' && define.amd) {
          define('webextension-polyfill', ['module'], defineFactory);
        } else if (typeof polyfillExports < 'u') {
          defineFactory(polyfillModule);
        } else {
          var fallbackModule = {
            exports: {},
          };
          defineFactory(fallbackModule);
          globalScope.browser = fallbackModule.exports;
        }
      })(
        typeof globalThis < 'u'
          ? globalThis
        ...
        function (moduleObject) {
          ...
          if (
            ...
            let buildBrowserApiWrapper = extensionApis => {
              let apiMetadata = {
                alarms: {
                  clear: {
                    minArgs: 0,
              ...
              let wrapMethod = (apiTarget, method, wrapper) =>
                new Proxy(method, {
                  apply(proxyTarget, thisArg, args) {
                    return wrapper.call(thisArg, apiTarget, ...args);
                  },
                });
              let hasOwnProp = Function.call.bind(
                  ...
                  get(proxyTarget, prop, receiver) {
                    ...
                    if (typeof wrappedValue == 'function') {
                      if (typeof wrappers[prop] == 'function') {
                        wrappedValue = wrapMethod(
                          target,
                          target[prop],
                          wrappers[prop],
                        );
                      } else if (hasOwnProp(metadata, prop)) {
                        let wrappedAsync = wrapAsyncFunction(
                          prop,
                          metadata[prop],
                        );
                        wrappedValue = wrapMethod(
                        ...
                        wrappedValue = wrappedValue.bind(target);
                      }
                    } else if (
                      typeof wrappedValue == 'object'
                      && wrappedValue !== null
                      && (hasOwnProp(wrappers, prop)
                        || hasOwnProp(metadata, prop))
                    ) {
                      wrappedValue = wrapObject(
                        wrappedValue,
                        wrappers[prop],
                        metadata[prop],
                      );
                    } else if (hasOwnProp(metadata, '*')) {
                      wrappedValue = wrapObject(
                        wrappedValue,
                        wrappers[prop],
                        metadata['*'],
              ...
              let onMessageWrappers = new DefaultWeakMap(messageListener =>
                typeof messageListener != 'function'
                  ? messageListener
                  : function (message, sender, sendResponse) {
                      let didRespond = !1;
                      let wrappedSendResponse;
                      let responsePromise = new Promise(resolveResponse => {
                      ...
                      let listenerResult;
                      try {
                        listenerResult = messageListener(
                          message,
                          sender,
                          wrappedSendResponse,
                        );
                      ...
                      let resultIsThenable =
                        listenerResult !== !0 && isThenable(listenerResult);
                      if (
                        listenerResult !== !0
                        && !resultIsThenable
                        && !didRespond
                      ...
                      sendResponseWhenReady(
                        resultIsThenable ? listenerResult : responsePromise,
                      );
                      return !0;
                    },
              );
              let handleSendMessageResponse = (
                ...
                if (extensionApis.runtime.lastError) {
                  ...
                  if (reply && reply.__mozWebExtensionPolyfillReject__) {
              ...
              let wrappedSendMessage = (name, metadata, target, ...args) => {
                if (args.length < metadata.minArgs) {
                  throw new Error(
                    `Expected at least ${metadata.minArgs} ${pluralizeArguments(metadata.minArgs)} for ${name}(), got ${args.length}`,
                  );
            ...
            moduleObject.exports = globalThis.browser;
          }
        },
      );
    },
  ...
  var initOption = defineLazyModule(() => {
    ...
    (function (optionNs) {
      function all() {
        for (var args = [], argIdx = 0; argIdx < arguments.length; argIdx++) {
          args[argIdx] = arguments[argIdx];
        }
        for (
          var values = [], valIdx = 0, argsList = args;
          valIdx < argsList.length;
          valIdx++
        ) {
...
`,
    ...
    OkImpl = (function () {
      ...
      OkCtor.prototype.unwrapErr = function () {
        throw new Error(
          'Tried to unwrap Ok: '.concat(stringifyValue(this.value)),
          {
            cause: this.value,
          },
        );
    ...
    (function (resultNs) {
      ...
      function any() {
        ...
        for (
          var errors = [], valIdx = 0, argsList = args;
          valIdx < argsList.length;
          valIdx++
        ) {
          var result = argsList[valIdx];
  ...
  var initAsyncResult = defineLazyModule(() => {
    ...
    generatorHelper = function (thisArg, body) {
      ...
      function step(operation) {
        ...
        for (
          ;
          iterator && ((iterator = 0), operation[0] && (state = 0)), state;
        ) {
          try {
            if (
              ((executing = 1),
              delegated
                && (opRecord =
                  operation[0] & 2
                    ...
                    : operation[0]
                      ? delegated.throw
                        || ((opRecord = delegated.return)
                          && opRecord.call(delegated),
                        0)
                      : delegated.next)
                && !(opRecord = opRecord.call(delegated, operation[1])).done)
            ) {
              return opRecord;
            }
            switch (
              ((delegated = 0),
              opRecord && (operation = [operation[0] & 2, opRecord.value]),
              operation[0])
            ) {
              case 0:
              case 1:
              ...
              default:
                if (
                  ((opRecord = state.trys),
                  !(opRecord =
                    opRecord.length > 0 && opRecord[opRecord.length - 1])
    ...
    AsyncResultImpl = (function () {
      ...
      AsyncResultCtor.prototype.andThen = function (nextFn) {
        var self = this;
